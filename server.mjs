...
const seed = {
  config:{eventName:"minicamp 2026",date:"2026-09-12/13",venue:"CSU Information Building 508",applicationOpen:true,applicationDeadline:"2026-09-05T23:59:00+08:00",resultDate:"2026-09-08T18:00:00+08:00",themeReveal:"Day 1 09:45",voteStartAt:null,voteOpen:false,juryWeight:40,participantWeight:60,starterPack:{title:"AI Coding Starter Pack",intro:"Prepare your tools and environment before the event.",tools:["Codex","Claude Code","Cursor","GitHub Copilot"],steps:["Install and sign in to your tool","Prepare Node.js, Python and Git","Ask AI to plan before splitting tasks","Share complete errors and verify the result"]}},
  applications:[
...
function body(req){return new Promise((resolve,reject)=>{let raw="";req.on("data",x=>{raw+=x;if(raw.length>1000000)reject(new Error("body too large"));});req.on("end",()=>{try{resolve(raw?JSON.parse(raw):{});}catch{reject(new Error("invalid JSON"));}});req.on("error",reject);});}
...
function votingIsOpen(){const start=Date.parse(db.config.voteStartAt||"");return Number.isFinite(start)&&Date.now()>=start;}
function addNotice(title,bodyText,type,target="ALL"){db.notices.unshift({id:makeId("NOTICE"),title,body:bodyText,type,target,readBy:[],createdAt:new Date().toISOString()});}
...
async function api(req,res,url){
  ...
  if(url.pathname==="/api/applications"&&method==="POST"){if(!db.config.applicationOpen)return fail(res,403,"application closed");const d=await body(req);if(!d.name||!d.studentId||!d.email||!d.phone||!d.motivation)return fail(res,400,"required fields missing");if(d.hasTeamIntent==="是"&&!String(d.teamIntentName||"").trim())return fail(res,400,"team intent name required");if(db.applications.some(x=>x.studentId===d.studentId||x.email.toLowerCase()===String(d.email).toLowerCase()))return fail(res,409,"duplicate application");const a={id:"MC26-"+String(1001+db.applications.length).padStart(4,"0"),...d,entryType:"个人报名",teamCode:"",teamIntentName:String(d.teamIntentName||"").trim(),skills:d.skills||[],status:"待审核",teamId:"",createdAt:new Date().toISOString()};db.applications.unshift(a);addNotice("Application received","Your application is received.","application",a.id);const t=makeToken();db.sessions[t]={role:"participant",userId:a.id,createdAt:Date.now()};await saveDb();return send(res,201,{application:safe(a),token:t});}
  if(url.pathname==="/api/auth/participant"&&method==="POST"){const d=await body(req);const c=String(d.contact||"").toLowerCase().replace(/[\s-]/g,"");const a=db.applications.find(x=>x.id.toUpperCase()===String(d.id||"").toUpperCase()&&[x.email,x.phone].some(v=>String(v||"").toLowerCase().replace(/[\s-]/g,"")===c));if(!a)return fail(res,401,"invalid participant credentials");const t=makeToken();db.sessions[t]={role:"participant",userId:a.id,createdAt:Date.now()};await saveDb();return send(res,200,{participant:safe(a),token:t});}
  if(url.pathname==="/api/auth/admin"&&method==="POST"){const d=await body(req);if(d.password!==adminPassword)return fail(res,401,"invalid admin password");const t=makeToken();db.sessions[t]={role:"admin",userId:"ADMIN",createdAt:Date.now()};return send(res,200,{token:t});}
  const me=person(req);
  ...
  if(url.pathname==="/api/me"&&method==="PATCH"){if(!me)return fail(res,401,"login required");const d=await body(req);if(d.hasTeamIntent==="是"&&!String(d.teamIntentName||"").trim())return fail(res,400,"team intent name required");d.entryType="个人报名";d.teamCode="";d.teamIntentName=String(d.teamIntentName||"").trim();Object.assign(me,d,{id:me.id,status:me.status,teamId:me.teamId});me.updatedAt=new Date().toISOString();addNotice("Profile updated","Your profile is waiting for review.","profile",me.id);await saveDb();return send(res,200,{participant:safe(me)});}
  ...
  if(url.pathname==="/api/teams"&&method==="GET")return send(res,200,{teams:db.teams.map(teamView)});
  if(url.pathname==="/api/teams"&&method==="POST"){if(!me)return fail(res,401,"login required");if(me.teamId||db.teams.some(item=>item.memberIds.includes(me.id)))return fail(res,409,"already belongs to a team");const d=await body(req);const team={id:"TEAM "+String(db.teams.length+1).padStart(2,"0"),code:"MC26-"+crypto.randomBytes(2).toString("hex").toUpperCase(),project:d.project||"Untitled",theme:d.theme||"TBD",memberIds:[me.id],status:"draft",locked:false};db.teams.push(team);me.teamId=team.id;me.teamCode=team.code;await saveDb();return send(res,201,{team:teamView(team)});}
  const tm=url.pathname.match(/^\/api\/teams\/([^/]+)\/(join|lock)$/);
  if(tm){const team=db.teams.find(x=>x.id===decodeURIComponent(tm[1]));if(!team)return fail(res,404,"team not found");if(tm[2]==="join"&&method==="POST"){if(!me)return fail(res,401,"login required");if(team.locked||team.memberIds.length>=5)return fail(res,409,"team is locked or full");const currentTeam=db.teams.find(item=>item.memberIds.includes(me.id)||item.id===me.teamId);if(currentTeam)return fail(res,409,currentTeam.id===team.id?"already in team":"leave current team first");team.memberIds.push(me.id);me.teamId=team.id;me.teamCode=team.code;await saveDb();return send(res,200,{team:teamView(team)});}if(tm[2]==="lock"&&method==="PATCH"){if(!me||!team.memberIds.includes(me.id))return fail(res,403,"team member required");if(team.memberIds.length<3||team.memberIds.length>5)return fail(res,400,"team must have 3 to 5 members");team.locked=true;team.status="locked";await saveDb();return send(res,200,{team:teamView(team)});}}
  if(url.pathname==="/api/ideas"&&method==="GET")return send(res,200,{ideas:db.ideas.filter(x=>x.status==="open")});
  if(url.pathname==="/api/ideas"&&method==="POST"){if(!me)return fail(res,401,"login required");const d=await body(req);if(!d.title||!d.summary)return fail(res,400,"idea title and summary required");const idea={id:makeId("IDEA"),title:d.title,summary:d.summary,theme:d.theme||"TBD",needs:d.needs||[],authorId:me.id,status:"open",createdAt:new Date().toISOString()};db.ideas.unshift(idea);await saveDb();return send(res,201,{idea});}
  ...
  if(url.pathname==="/api/projects"&&method==="POST"){if(!me)return fail(res,401,"login required");const team=db.teams.find(x=>x.id===me.teamId);if(!team)return fail(res,403,"join a team first");const d=await body(req);const project={id:makeId("PROJECT"),teamId:team.id,...d,members:d.members||team.memberIds.map(mid=>({name:db.applications.find(x=>x.id===mid)?.name||"member",role:""})),aiTools:d.aiTools||[],status:"draft",createdAt:new Date().toISOString()};db.projects.push(project);await saveDb();return send(res,201,{project:projectView(project)});}
  ...
  if(url.pathname==="/api/votes"&&method==="POST"){const jury=admin(req);if(!votingIsOpen())return fail(res,403,"voting not open");if(!jury&&!me)return fail(res,401,"login required");const d=await body(req),role=jury?"jury":"participant",voterId=jury?"ADMIN":me.id;if(db.votes.some(x=>x.voterId===voterId&&x.role===role))return fail(res,409,"vote already submitted");const selections=normalizeVoteSelections(d.selections,role);if(!selections)return fail(res,400,"invalid vote selections");const allowed=new Set(db.projects.filter(x=>x.status==="published").map(x=>x.id));if(selections.some(x=>!allowed.has(x.projectId)))return fail(res,400,"unpublished project in vote");if(me&&selections.some(x=>db.projects.find(p=>p.id===x.projectId)?.teamId===me.teamId))return fail(res,400,"cannot vote for your team");db.votes.push({id:makeId("VOTE"),voterId,role,selections,createdAt:new Date().toISOString()});await saveDb();return send(res,201,{ok:true});}
  if(url.pathname==="/api/organizer/summary"&&method==="GET")return send(res,200,{config:{eventName:db.config.eventName,date:db.config.date,venue:db.config.venue,applicationOpen:db.config.applicationOpen,applicationDeadline:db.config.applicationDeadline,resultDate:db.config.resultDate},metrics:{applications:db.applications.length,accepted:db.applications.filter(x=>x.status==="已录取").length,pending:db.applications.filter(x=>x.status==="待审核").length,teams:db.teams.length,publishedProjects:db.projects.filter(x=>x.status==="published").length},teams:db.teams.map(t=>({id:t.id,project:t.project,theme:t.theme,status:t.status,memberCount:t.memberIds.length})),projects:db.projects.filter(x=>x.status==="published").map(p=>({id:p.id,projectName:p.projectName,theme:p.theme,tagline:p.tagline,demoUrl:p.demoUrl})),notices:db.notices.filter(x=>x.target==="ALL").slice(0,10).map(x=>({title:x.title,body:x.body,type:x.type,createdAt:x.createdAt}))});
  if(url.pathname==="/api/admin/summary"&&method==="GET"){if(!admin(req))return fail(res,401,"admin required");return send(res,200,{applications:db.applications.map(safe),teams:db.teams.map(teamView),projects:db.projects.map(projectView),notices:db.notices,results:calcResults(),awards:resolvedAwards(),config:{...db.config,voteOpen:votingIsOpen()}});}
